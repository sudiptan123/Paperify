...
module.exports.getColablist = async (req, res, next) => {
    
    

    
    db.query(
        "SELECT u.id as uId, u.name as uName, u.phone as uPhone, u.email as uEmail, b.id as bId, b.title as bTitle, b.content as bContent, b.catagory as bCatagory, b.useremail as bEmail FROM colab b JOIN user u ON b.userId = u.id",
        (Err, result) => {
            if (Err) throw Err;
            if (!result[0]) {
                req.colablist = "";
...
module.exports.deleteColab = async (req, res, next) => {
    if (req.role === "ADMIN" || req.role === "USER") {
